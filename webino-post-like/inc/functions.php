...
function webino_post_like_install()
{
    global $wpdb;

    $table_post_likes = $wpdb->prefix . 'wpl_post_likes';
...
function webino_post_like_script()
{
    wp_enqueue_script(
        'wpl_script',
        WEBINO_POST_LIKE_JS_URL . 'post-like.js',
        ['jquery'],
        defined('WP_DEBUG') && WP_DEBUG ? time() : WEBINO_POST_LIKE_VERSION,
        true
    );
...
function webino_post_like_style()
{
?>
    <style>
        button.like-post {
...
function webino_post_like_button($content)
{
    $like_text = __('Like', 'webino');
    $post_id = get_the_ID();


    $like_count = webino_post_like_get_post_like($post_id);
    $liked_class = webino_post_like_is_like_post($post_id, get_current_user_id()) ? 'post-liked' : '';
...
function webino_post_like_ajax_callback()
{
    ...
    if (!isset($_POST['_wpnonce']) || !wp_verify_nonce($_POST['_wpnonce'], 'post-like' . $post_id)) {
        wp_die('cant pass nonce');
        wp_send_json_error([
            'message' => 'Forbideen, nonce is invalid',
            'code' => '403'
        ], 401);
    }

    $liked = webino_post_like_do_like($post_id, get_current_user_id(), $like);
